...
export function calc_py(vertices: any) {
	return checkPyScriptReady(() => {
		const py_main = pyscript.interpreter.globals.get('main');
		const result = py_main(JSON.stringify(vertices));
		return JSON.parse(result); // py_main.py에서 텍스트 포맷 result를 다시 json형태로 변경
	});
}